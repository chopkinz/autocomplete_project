   ...
   public static class TrieAutocomplete implements Autocompletor {
      ...
      private void add(String word, double weight) {
         ...
         for (int a = 0; a < word.length(); a++) {
            ...
            Node next = temp.getChild(character);
            
            // check if current child node is null 
            if (next == null) {
               next = new Node(character, temp, weight); // new child node 
               temp.children.put(character, next); // add new node to tree 
            }
            
            // check max weight 
            if (weight > temp.mySubtreeMaxWeight) {
               temp.mySubtreeMaxWeight = weight;
            }
            temp = next;
         }
         // update values for added node
         temp.setWord(word);
      ...
      public Iterable<String> topMatches(String prefix, int k) {
         ...
         ArrayList<String> noWordsList = new ArrayList<String>(); 

         PriorityQueue<Node> priorityQueue = new PriorityQueue<Node>(new Node.ReverseSubtreeMaxWeightComparator());
            
         
         for (int a = 0; a < prefix.length(); a++) {
            character = prefix.charAt(a);
            temp = temp.getChild(character);
            
            // return empty string if child = null
            if (temp == null) {
               return noWordsList;
            }
         }

         // add words to priorityQueue
         ...
         while (!priorityQueue.isEmpty()) {
            ...
            if (wordList.size() >= k) {
               ...
               /*
               * learned how to do this on GeeksForGeeks
            ...
            for (Node no : temp.children.values()) {
            ...
            */
         for (int b = 0; b < Math.min(k,wordList.size()); b++) {
            ouput.add(wordList.get(b).myWord);
         }
         return ouput;            
      }      
      ...
      public String topMatch(String prefix) {
         ...
         for (int a = 0; a < prefix.length(); a++) {
            ...
            /* 
            * Learned how to implement this from GeeksforGeeks:
         ...
         // if no word with largest weight exists, return empty string 
         if (matchFound == false) {
            return string;
         }
         while (temp.mySubtreeMaxWeight > temp.getWeight() && !temp.isWord) {
            for (Node node : temp.children.values()) {
       ...
       */
      public double weightOf(String term) {
         // New node that tempently points to the root node.
         Node temp = myRoot;
